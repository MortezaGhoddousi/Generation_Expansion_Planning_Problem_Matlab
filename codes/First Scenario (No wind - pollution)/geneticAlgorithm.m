...
res = reshape(inputArg1, 10, 3);
disp (res)

% Define Parameters
num_years = 10; % Planning horizon in years
num_plants = 3; % Number of potential new plants
initial_capacity = [100, 150, 100]; % Initial capacity of existing plants (MW)
inv_cost = [1000, 800, 900]; % Investment cost for new plants ($/MW)
fuel_cost = [50, 60, 55]; % Fuel cost for plants ($/MWh)
o_m_cost = [10, 12, 10]; % Operation & maintenance cost ($/MWh)
pollution_cost = [1000, 1200, 1000]; % Pollution cost ($/MWh)
pollution_limit = [1000, 800, 1000]; % Pollution limit (units)
demand_growth = 0.1; % Annual demand growth rate
initial_demand = 600; % Initial demand (MW)
pollution_rate = [1.5, 1.6, 1.8]; % Pollution rate per plant (units/MWh)
