...
res = reshape(inputArg1, 10, 5);
disp (res)

% Define Parameters
num_years = 10; % Planning horizon in years
num_plants = 5; % Number of potential new plants
initial_capacity = [100, 150, 90, 110, 200]; % Initial capacity of existing plants (MW)
inv_cost = [1000, 800, 700, 1100, 900]; % Investment cost for new plants ($/MW)
fuel_cost = [50, 60, 5, 55, 70]; % Fuel cost for plants ($/MWh)
o_m_cost = [10, 12, 8, 11, 15]; % Operation & maintenance cost ($/MWh)
pollution_cost = [100, 120, 10, 90, 50]; % Pollution cost ($/MWh)
demand_growth = 0.1; % Annual demand growth rate
initial_demand = 600; % Initial demand (MW)
pollution_rate = [1.5, 1.6, 0.1, 1, 0.8]; % Pollution rate per plant (units/MWh)
...
fuel_usage = [0.8, 0.9, 0.1, 0.6, 0.8]; % Fuel usage (units/MWh)
